...
{    
    public static void main(String args[])
    {
        ...
        if(args.length < 3)    {
            ...
            System.out.println("Calcolo numeri primi in corso...");
            service1.calcolaPrimi(Integer.parseInt(args[1]), Integer.parseInt(args[2]));
        }
    }
}
...
class RESTAPI
{
    ...
    void calcolaPrimi(int min, int max) {
        URL u = null;
        try {
            u = new URL("http://"+server+":8000/calcola-primi?param1="+min+"&param2="+max);
            System.out.println("URL aperto: " + u);
        } catch (MalformedURLException e) {
            System.out.println("URL errato: " + u);
            return;
        ...
        try {
            ...
            c.connect();
            BufferedReader b = new BufferedReader(new InputStreamReader(c.getInputStream()));
            System.out.println("Lettura dei dati (potrebbe richiedere tempo)...");
            String s;
            while ((s = b.readLine()) != null) {
                // Per evitare di stampare 1000000 di righe, stampiamo a terminale, ma
                // potremmo anche ometterlo. Lo lasciamo per debug e coerenza con la somma.
                System.out.println(s);
            }
